...
**类型**: `Array<Object>`
**默认值**: `[]`
**说明**: 存储用户创建的自定义基金分组信息及系统预置分组（如“自选”），同时负责记录各分组在系统中的展示排序次序。
**云端同步**: 是
**导入/导出**: 是
...
[
  {
    id: string,           // 分组唯一标识（预置“自选”分组固定为 'fav'）
    name: string,         // 分组名称（预置“自选”分组固定为 '自选'）
    isPreset: boolean,    // 可选，是否为系统预置分组（如 'fav' 为 true）
    codes: Array<string>  // 分组内的基金代码列表（对于 'fav' 分组通常由 favorites Set 维护，此处默认为 []）
  }
]
```
...
- 显示分组横向标签栏及下拉选择菜单（按数组次序全局展示）
- 分组管理（添加、编辑、删除以及对自选及自定义分组进行拖拽排序）
- 导入/导出配置时包含
